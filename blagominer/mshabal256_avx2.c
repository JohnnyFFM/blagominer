...
//Johnnys double pointer no memmove  burst mining only optimisation functions (tm) :-p
  ...
  static void
	  mshabal256_compressA_fast(mshabal256_context_fast *sc,
...
#define M(i)   _mm256_load_si256(u.data + (i))
	  ...
	  while (num-- > 0) {
		  ...
		  for (j = 0; j < 64 * MSHABAL256_FACTOR/2; j += 4 * MSHABAL256_FACTOR) {
			  size_t o = j / MSHABAL256_FACTOR;
			  u.words[j + 0] = *(u32 *)(buf0 + o);
			  u.words[j + 1] = *(u32 *)(buf0 + o);
			  u.words[j + 2] = *(u32 *)(buf0 + o);
			  u.words[j + 3] = *(u32 *)(buf0 + o);
			  u.words[j + 4] = *(u32 *)(buf0 + o);
			  u.words[j + 5] = *(u32 *)(buf0 + o);
			  u.words[j + 6] = *(u32 *)(buf0 + o);
			  u.words[j + 7] = *(u32 *)(buf0 + o);
...
#undef M
  ...
  static void
	  mshabal256_compressB_fast(mshabal256_context_fast *sc,
		  const unsigned char *buf0, const unsigned char *buf1,
		  const unsigned char *buf2, const unsigned char *buf3,
		  const unsigned char *buf4, const unsigned char *buf5,
		  const unsigned char *buf6, const unsigned char *buf7,
...
#undef M
  ...
  static void
	  mshabal256_compressC_fast(mshabal256_context_fast *sc,
		  const unsigned char *buf0, const unsigned char *buf1,
		  const unsigned char *buf2, const unsigned char *buf3,
		  const unsigned char *buf4, const unsigned char *buf5,
		  const unsigned char *buf6, const unsigned char *buf7,
...
#undef M
  ...
  static void
	  mshabal256_compressAC_fast(mshabal256_context_fast *sc,
...
#undef M
  ...
  void
	  mshabal256_fast(mshabal256_context_fast *sc,
		  ...
		  size_t len)
  {
	  mshabal256_compressA_fast(sc, (const unsigned char *)data0, (const unsigned char *)xdata0, (const unsigned char *)xdata1, (const unsigned char *)xdata2, (const unsigned char *)xdata3, (const unsigned char *)xdata4, (const unsigned char *)xdata5, (const unsigned char *)xdata6, (const unsigned char *)xdata7, 1);
  }
