...
//Johnnys double pointer no memmove  burst mining only optimisation functions (tm) :-p

  static void
	  mshabal256_compress_fast(mshabal256_context_fast *sc,
		  const unsigned char *buf0, const unsigned char *buf1,
		  const unsigned char *buf2, const unsigned char *buf3,
		  const unsigned char *buf4, const unsigned char *buf5,
		  const unsigned char *buf6, const unsigned char *buf7,
		  const unsigned char *xbuf0, const unsigned char *xbuf1,
		  const unsigned char *xbuf2, const unsigned char *xbuf3,
		  const unsigned char *xbuf4, const unsigned char *xbuf5,
		  const unsigned char *xbuf6, const unsigned char *xbuf7,
		  size_t num)
  {
	  _mm256_zeroupper();
	  union {
...
#define M(i)   _mm256_load_si256(u.data + (i))
	  ...
	  while (num-- > 0) {
		  ...
		  for (j = 0; j < 64 * MSHABAL256_FACTOR/2; j += 4 * MSHABAL256_FACTOR) {
			  ...
			  u.words[j + 6] = *(u32 *)(buf6 + o);
			  u.words[j + 7] = *(u32 *)(buf7 + o);
			  u.words[j + 0 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 1 + 64] = *(u32 *)(xbuf1 + o);
			  u.words[j + 2 + 64] = *(u32 *)(xbuf2 + o);
			  u.words[j + 3 + 64] = *(u32 *)(xbuf3 + o);
			  u.words[j + 4 + 64] = *(u32 *)(xbuf4 + o);
			  u.words[j + 5 + 64] = *(u32 *)(xbuf5 + o);
			  u.words[j + 6 + 64] = *(u32 *)(xbuf6 + o);
			  u.words[j + 7 + 64] = *(u32 *)(xbuf7 + o);
		  }
...
#undef M
  ...
  void
	  mshabal256_fast(mshabal256_context_fast *sc,
		  void *data0, void *data1, void *data2, void *data3,
		  void *data4, void *data5, void *data6, void *data7,
		  ...
		  size_t len)
  {
	  mshabal256_compress_fast(sc, (const unsigned char *)data0, (const unsigned char *)data1, (const unsigned char *)data2, (const unsigned char *)data3, (const unsigned char *)data4, (const unsigned char *)data5, (const unsigned char *)data6, (const unsigned char *)data7, (const unsigned char *)xdata0, (const unsigned char *)xdata1, (const unsigned char *)xdata2, (const unsigned char *)xdata3, (const unsigned char *)xdata4, (const unsigned char *)xdata5, (const unsigned char *)xdata6, (const unsigned char *)xdata7, 1);
  }

  void
	  mshabal256_close_fast(mshabal256_context_fast *sc,
		  ...
		  unsigned n)
  {
	  ...
	  for (z = 0; z < 4; z++) {
		  mshabal256_compress_fast(sc, (const unsigned char *)buf0, (const unsigned char *)buf1, (const unsigned char *)buf2, (const unsigned char *)buf3, (const unsigned char *)buf4, (const unsigned char *)buf5, (const unsigned char *)buf6, (const unsigned char *)buf7, (const unsigned char *)xbuf0, (const unsigned char *)xbuf1, (const unsigned char *)xbuf2, (const unsigned char *)xbuf3, (const unsigned char *)xbuf4, (const unsigned char *)xbuf5, (const unsigned char *)xbuf6, (const unsigned char *)xbuf7, 1);
		  if (sc->Wlow-- == 0)
			  sc->Whigh--;
	  }
	  out_size_w32 = sc->out_size >> 5;
