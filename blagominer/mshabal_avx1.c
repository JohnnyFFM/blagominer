...
#define M(i)   _mm_load_si128(u.data + (i))
	  ...
	  while (num-- > 0) {
		  ...
		  for (j = 0; j < 64 / 2; j += 4) {
			  ...
			  u.words[j + 0 + 32] = *(u32 *)(xfub0 + o);
			  u.words[j + 1 + 32] = *(u32 *)(xfub1 + o);
			  u.words[j + 2 + 32] = *(u32 *)(xfub2 + o);
			  u.words[j + 3 + 32] = *(u32 *)(xfub3 + o);
		  }
...
#define M(i)   _mm_load_si128(u.data + (i))
	  ...
	  while (num-- > 0) {
		  ...
		  for (j = 0; j < 64 / 2; j += 4) {
			  ...
			  u.words[j + 2] = *(u32 *)(buf2 + j);
			  u.words[j + 3] = *(u32 *)(buf3 + j);
			  u.words[j + 0 + 32] = *(u32 *)(xbuf0 + j);
			  u.words[j + 1 + 32] = *(u32 *)(xbuf0 + j);
			  u.words[j + 2 + 32] = *(u32 *)(xbuf0 + j);
			  u.words[j + 3 + 32] = *(u32 *)(xbuf0 + j);
		  }
...
#undef M
		  ...
		  unsigned n)
  {
	  ...
	  avx1_mshabal_compress_fast(sc, (const unsigned char *)fub0, (const unsigned char *)xfub0, (const unsigned char *)xfub1, (const unsigned char *)xfub2, (const unsigned char *)xfub3, (const unsigned char *)buf0, (const unsigned char *)buf1, (const unsigned char *)buf2, (const unsigned char *)buf3, (const unsigned char *)xbuf0, 1);
