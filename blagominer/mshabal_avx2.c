...
#define ROTL32(x, n)   T32(((x) << (n)) | ((x) >> (32 - (n))))
  ...
  static void
    avx2_mshabal_compress(mshabal256_context *sc,
...
#undef M
	...
	avx2_mshabal_init(mshabal256_context *sc, unsigned out_size)
  {
	...
	avx2_mshabal_compress(sc, sc->buf0, sc->buf1, sc->buf2, sc->buf3, sc->buf4, sc->buf5, sc->buf6, sc->buf7, 1);
    for (u = 0; u < 16; u++) {
    ...
    size_t len)
  {
    ...
    num = 1;
    if (num != 0) {
		avx2_mshabal_compress(sc, (const unsigned char *)data0, (const unsigned char *)data1, (const unsigned char *)data2, (const unsigned char *)data3, (const unsigned char *)data4, (const unsigned char *)data5, (const unsigned char *)data6, (const unsigned char *)data7, num);
    ...
    void *dst4, void *dst5, void *dst6, void *dst7)
  {
    ...
    for (z = 0; z < 4; z++) {
      avx2_mshabal_compress(sc, sc->xbuf0, sc->xbuf1, sc->xbuf2, sc->xbuf3, sc->xbuf4, sc->xbuf5, sc->xbuf6, sc->xbuf7, 1);
      if (sc->Wlow-- == 0)
        sc->Whigh--;
    }
...
//Johnnys double pointer no memmove no register buffering burst mining only optimisation functions (tm) :-p
   ...
   static void
	  avx2_mshabal_compress_fast(mshabal256_context_fast *sc,
...
#define M(i)   _mm256_load_si256(u.data + (i))
	  ...
	  while (num-- > 0) {
		  ...
		  for (j = 0; j < 64 * MSHABAL256_FACTOR / 2; j += 4 * MSHABAL256_FACTOR) {
			  size_t o = j / MSHABAL256_FACTOR;
			  u.words[j + 0] = *(u32 *)(fub0 + o);
			  u.words[j + 1] = *(u32 *)(fub0 + o);
			  u.words[j + 2] = *(u32 *)(fub0 + o);
			  u.words[j + 3] = *(u32 *)(fub0 + o);
			  u.words[j + 4] = *(u32 *)(fub0 + o);
			  u.words[j + 5] = *(u32 *)(fub0 + o);
			  u.words[j + 6] = *(u32 *)(fub0 + o);
			  u.words[j + 7] = *(u32 *)(fub0 + o);
			  u.words[j + 0 + 64] = *(u32 *)(xfub0 + o);
			  u.words[j + 1 + 64] = *(u32 *)(xfub1 + o);
			  u.words[j + 2 + 64] = *(u32 *)(xfub2 + o);
			  u.words[j + 3 + 64] = *(u32 *)(xfub3 + o);
			  u.words[j + 4 + 64] = *(u32 *)(xfub4 + o);
			  u.words[j + 5 + 64] = *(u32 *)(xfub5 + o);
			  u.words[j + 6 + 64] = *(u32 *)(xfub6 + o);
			  u.words[j + 7 + 64] = *(u32 *)(xfub7 + o);
		  }
...
// Round 2-5
	  for (int k = 0;k < 4;k++)
	  {
		  for (j = 0; j < 64 * MSHABAL256_FACTOR / 2; j += 4 * MSHABAL256_FACTOR) {
			  ...
			  u.words[j + 1 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 2 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 3 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 4 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 5 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 6 + 64] = *(u32 *)(xbuf0 + o);
			  u.words[j + 7 + 64] = *(u32 *)(xbuf0 + o);
		  }
