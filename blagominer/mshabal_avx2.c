...
#undef M
    ...
    const void *data2, const void *data3, size_t len)
  {
    ...
    num = len >> 6;
    if (num != 0) {
      avx2_mshabal_compress(sc, (const unsigned char *)data0, (const unsigned char *)data1, (const unsigned char *)data2, (const unsigned char *)data3, num);
  ...
  /* see shabal_small.h */
  void
    avx2_mshabal_close(mshabal_context *sc,
